...
Foufold plots are automatically stratified over the levels of a third feature.
...
A pivot table creates rows for combos of > 1 feature. This is a great way to extend simple ca and is much more interpretable than mjca.

```{r sts_ca_struc, echo=TRUE}
...
To allow for estimation, we constrain: $\sum^I_{i = 1} \lambda^{A}_{i} = 0,\ \sum^J_{j = 1} \lambda^{B}_{i} = 0,\ \sum^I_{i = 1} \lambda^{AB}_{ij} = \sum^J_{j = 1} \lambda^{AB}_{ij} = 0$. There are $(I-1) + (J-1) + (I-1)(J-1)$ degrees of freedom.
