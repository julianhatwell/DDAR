...
To provide a high-level overview of some less well-known, visual statitistical tools that can be applied to very common tasks in Machine Learning practice and research.

**Objectives:**

* Present some classical statistical techniques that are not usually included in a basic stats curriculum
...
* Plotting the square root transformation of the frequency provides more detail on the small numbers in the right tail of these charts. Log transformation would also work but square root makes distances proportional to $\chi^2$ residuals, which is a very useful property. The rootogram visualisations use square root transformation by default. So, this is also applied manually where necessary for comparison.
...
A very naive way to estimate the distribution is just to take the mean visually compare a poisson distribution with this value for $\lambda$.
...
The $\chi^2$ test shows a poor fit. We reject the null hypothesis that this is a sample from a poisson distributed random variable. However, we don't know much about what caused this result.

```{r fed_goodfit_pois_plot}
plot(Fed_fit0, type = "hanging", shade = TRUE)
```

However, the rootogram plot makes it very clear what the problem is by showing how the variable diverges from the expectations. By hanging each bar top down from the expected distribution, we just need to look at how the bottoms of the bars miss the $y = 0$ reference line. These are proportional to the residuals. The contribution of each bar to the $\chi^2$ statistic is also made clear by the shading.

We can try fitting a poisson using a different estimation method, minimising the $\chi^2$ errors
...
The $\chi^2$ test actually shows a much worse fit.

```{r fed_goodfit_pois_mplot}
plot(Fed_fit0, type = "hanging", shade = TRUE)
```

The rootogram shows that the residuals have just been shuffled around so they balance by sign. Clearly poisson is not the right choice.
...
The $\chi^2$ goodness of fit test result is not significant. We accept that null hypothesis that this sample is from a negative binomial distribution. The plot speaks for itself now that the bars drop close to the $y = 0$ reference line, with only insignificant residuals. We have also estimated the parameters and can now use this distribution to assess a different text.
...
The $\chi^2$ result is significant, and strong evidence that this new text is not from the same distribution. However, on it's own, it can't provide detailed information. 

```{r fit_nonfed_plot}
plot(Fed_fit3, type = "hanging", shade = TRUE)
```

The rootogram hangs the frequency bars off the distribution that we just estimated and we can now see from the pattern of residuals that the word 'may' seems to appear more frequently in this text. Specifically, the tail, with 3-5 occurences per 200 words is fatter, while blocks where this word is absent are fewer.
...
That's all well and good, but what if class labels are related to each other? If they have a natural order such [*least, middle, most*] then you have an additional complication. You want your model to predict as well as possible, but to make an off-by-one error isn't as bad as a full opposite error. Also, the classification distribution might be biased, giving different skew and kurtosis (flattened or polarised) by the predictive model.
...
On this dataset, we can't say which is the true and which is predicted because they're both independent raters, but for now let's assume that Winnipeg Neurologist is the true class and New Orleans is the predicted class. The highest accuracy we could expect to get with constant model that always makes the same rating is the proportion of the majority true class. This would give a score of  $\kappa = 0$:
...
Here the grid lines between cells are no longer continuous because the cells are out of proportion with the marginal totals. Bigger cells means that the combination is over-represented and smaller cells the reverse. 
...
Dimension 2 is forced to capture differences between ABC and CBS schedules on the other nights as well as possible.

Now we have identified this difficulty, we can untangle the problem by making a joint analysis of time and day. This will produce a plot with a point for each time slot over the week and should untangle the effect of people channel hopping from the "Friends, Seinfeld and ER" effect. It turns out to be very easy to do (see the code for yourself).
...
As you can see, the small channel hop from NBC on Thursday has moved onto Dimension 2, along with the other variations in within-evening viewing patterns. On the previous plot, there is a very slight affinity 8pm-CBS, and 9pm-ABC (I'm sure you didn't notice that before now). We can see clearly here, something of great interest at 8pm, Monday, but not later on the same evening and not again at the same time later in the week. We also can see several 9pm slots clustered around ABC.

An interesting feature of this new analysis, is that distance from the origin is proportional to the size of positive residual. What does this mean? Well for example, from 10pm viewing figures dwindle over all, independent of day and channel. Thursday night, 10pm, NBC has massive audience figures relative to what's normally expected for that time slot. This inertia moves the point furthest from the origin out of all the nightly time slots. Monday at 8pm on CBS is notable for the same reasons.
