...
The plots above place all the focus on the numeric variables, slicing it into groups by the various discrete variables. We would like charts where the focus is on the discrete variables; something that provides more information about the discrete variables themselves.
...
Most people with some stats knowledge are aware of the log odds $\log(\frac{p}{1-p})$ from logistic regression. The odds is itself the ratio $o = \frac{p}{q}$ for $p$, the proportion of yes and $q$, the proportion of no in any binary situation, like a coin flip. 
...
* Confidence intervals for statistical results are also symmetrical in the log odds domain, facilitating comparisons between levels of a third feature $c$ as both visually intuitive and statistically robust.
...
However, these feature importance measures tend to be crude. The feature is important, but why? All feature importance really tells you is that the model would be less accurate if you excluded that feature. It doesn't tell anything how that feature is helping.

### When would I use this?

When you are evaluating your classification model performance with a confusion matrix and you have a reason to believe that one or more categorical variables contributed to good/bad perforemance.

### Case Study: Using Random Forest Importance

We will now do some ML and train a random forest model to predict whether each family in a sample will receive a nursery place from the Slovenian state. The target variable is *decision*
...
# ggplot
# too much fuss, also the wrong diagonal
...
We can see from this plot that the health variable seems to hold a majority of the predictive power for this model. Almost all the information is in this and the next three most important variables.
...
A situation like this is quite unusual in the real world, and truthfully, should be identified during exploratory analysis. In binary classification, this could never happen, else there would be no ML to do. Problems are often organised to that the two classes are balanced. How could this slicing technique help?
