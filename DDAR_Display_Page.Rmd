...
* $H_0 =$ In a random sample of the population, there is an equal chance of belonging to any category of feature A.
...
In this workshop, we will focus on intuitive, visual tools that can reveal very interesting patterns in categorical data. We will only introduce formal statistics and tests where absolutely necessary.

## What we won't cover

Bear in mind, this is just an overview! Our seminar session is just long enough for a taster and not much depth.

We won't cover explanatory and predictive modeling. This includes logistic regression for binary classification, which most will be familiar with. It falls squarely under both DDA and Machine Learning, and there are related or equivalent techniques for other kinds of classifiation (ordinal, polytomous/multi-class and counts. This is all out of scope for today because there isn't time to review the underlying theories.
...
* Demonstrate rigorous and robust, yet visually intuitive reporting of machine learning project results in:
...
That's a story about the BI, management reports and forecasting work that I used to do. Now I am involved in (or evolved into) a Machine Learning researcher and practitioner, I face completely new tasks, but I keep noticing that the old tools and techniques are still applicable. I wanted to share these ideas with you and I hope you also find them useful.
...
The $\chi^2$ result is significant, and therefore evidence that this new text is not from the same distribution. However, on it's own, it can't provide detailed information. 

```{r fit_nonfed_plot}
plot(Fed_fit3, type = "hanging", shade = TRUE)
```

The rootogram hangs the frequency bars off the distribution that we just estimated and we can see now see from the pattern of residuals that the word 'may' seems to appear more frequently in this text. Specifically, the tail, with 3-6 occurences per 200 words is fatter, while blocks where this word is absent are fewer.
...
We can still see the general opposite corner pattern, and some additional detail regarding gender. This type of plot is still fairly intuitive at 4 dimensions. More dimensions can be added by other means. There are several options:
...
There is a whole methodology around loglinear modeling which allows you to make make statistical hypothesis tests about complex n-way relationships between features. There is enough material to write several books on this topic!
...
CBS had the lion's share on Monday and ABC has Wednesday and Friday but neither channel had any massive crowd pleasers, the way NBC dominated Thursday nights.

Audience was split evenly on Tuesday.
